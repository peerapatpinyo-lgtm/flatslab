...
def render(c1_w, c2_w, L1, L2, lc, h_slab, fc):
    ...
    st.markdown("Use these **Stiffness (K)** values for your Structural Analysis Software.")
    ...
    with c2:
        ...
        st.metric("DF Column (Total)", f"{df_col:.3f}")
        st.progress(df_slab)
        
    st.info("""
    **Equivalent Column (Kec):** Combines the stiffness of the columns ($\Sigma K_c$) and the torsional members ($K_t$).
    This accounts for the flexibility of the slab-column connection.
    """)
