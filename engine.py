...
def calculate_detailed_slab(lx, ly, h_mm, c1_mm, c2_mm, fc_ksc, fy_ksc, sdl, ll, cover_mm, pos="Interior", dl_factor=1.2, ll_factor=1.6):
    ...
    ln_check = max(lx, ly) - c1
    h_min_req = (ln_check * 1000) / 30 if pos != "Interior" else (ln_check * 1000) / 33
    h_warning = ""
    if h_mm < h_min_req:
        h_warning = f"Warning: Thickness ({h_mm}mm) < Min Req ({h_min_req:.0f}mm). Check Deflection!"

    # --- 3. Loading Calculation (Updated Factors) ---
    sw = h * 2400
    ...
    def get_punching_data(temp_h_mm):
        ...
        return {"vu": vu_kg, "phi_vc": phi_vc_kg, "bo": bo, "d": t_d, "vc_mpa": vc_mpa, 
    ...
    for key, coeff in m_coeffs.items():
        m_strip = coeff * mo
        as_req = (m_strip * 100) / (0.9 * fy_ksc * (p['d'] * 100 * 0.9))
        rebar_out[key] = max(as_req / ly, as_min)

    return {
        "loading": {
            "qu": qu, "sw": sw, "dl_fact": w_dl_factored, "ll_fact": w_ll_factored,
            "trib_area": tributary_area, "total_load": total_load_kg, "total_load_ton": total_load_ton,
        ...
        "ratio": p['vu']/p['phi_vc'] if p['phi_vc']>0 else 0
    }
