...
def draw_section(h_mm, cover_mm, c1_mm, ln_m, d_mm, top_label, bot_label):
    ...
    ax.text(0, y_top + 0.05, f"Top: {top_label}", color='blue', ha='center', fontweight='bold', fontsize=10)
    
    # Bot Bar
    ax.plot([-w+0.1, w-0.1], [y_bot, y_bot], color='green', linewidth=3, label='Bot Bar')
    ax.text(1.0, y_bot - 0.08, f"Bot: {bot_label}", color='green', ha='center', fontweight='bold', fontsize=10)
    ...
    # d effective
    ax.annotate(f"d={d_mm:.0f}mm", xy=(-w+0.5, h-cover), xytext=(-w+0.5, 0),
