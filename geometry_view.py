...
def plot_geometry_detailed(L1, L2, c1, c2, h_slab, lc, Ic_val):
    """
    Generate a figure with 2 subplots: Top View (Plan) and Section View.
    """
    ...
    # Dimensions Function
    def draw_dim(ax, p1, p2, text, off_x=0, off_y=0, color='#0d6efd'):
        ax.annotate('', xy=p1, xytext=p2, arrowprops=dict(arrowstyle='<|-|>', color=color, lw=1))
        mx, my = (p1[0]+p2[0])/2 + off_x, (p1[1]+p2[1])/2 + off_y
        ax.text(mx, my, text, ha='center', va='center', color=color, fontweight='bold', fontsize=9,
    ...
    ax2.axhline(y=0, color='black', lw=2) # Ground Line
    ...
    # Dims
    draw_dim(ax2, (c1_m*1.5, 0), (c1_m*1.5, lc), f"lc = {lc} m", off_x=0.3, color='#198754')
    draw_dim(ax2, (-L1*0.2, lc-h_m), (-L1*0.2, lc), f"h = {h_slab} cm", off_x=-0.3, color='#dc3545')
